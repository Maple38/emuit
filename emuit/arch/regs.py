...
class EmuRegs:
    ...
    def _reg_id_by_name(self, register: str):
        ...
        if (
            self._arch.uc_architecture == uc.unicorn_const.UC_ARCH_X86
            and register.startswith("*")
        ):
            register = {
                uc.unicorn_const.UC_MODE_64: "R",
                uc.unicorn_const.UC_MODE_32: "E",
                uc.unicorn_const.UC_MODE_16: "",
            }[self._arch.uc_mode] + register[1:]
        # workaround for PPC arch
        elif (
            self._arch.uc_architecture == uc.unicorn_const.UC_ARCH_PPC
            and register.startswith("R")
        ):
            register = register[1:]
    ...
    def __setitem__(self, destination: str, value: int):
